...
updatedDate: 2026-03-12 15:54:00
tags:
  - Stack
  - Monotonic Stack
---
...
1. 栈为空，直接入栈 2。栈为：`[2]`
2. 1 比 2 小，不满足单调性，弹出 2，入栈 1。栈为：`[1]`
3. 5 比 1 大，满足单调性，入栈 5。栈为：`[1, 5]`
4. 6 比 5 大，满足单调性，入栈 6。栈为：`[1, 5, 6]`
5. 2 比 6 小，不满足单调性，弹出 6。栈为：`[1, 5]`
6. 2 比 5 小，不满足单调性，弹出5，入栈 2。栈为：`[1, 2]`
7. 3 比 2 大，满足单调性，入栈 3。栈为：`[1, 2, 3]`
...
1. 根据题目要求，我们需要找到右侧第一个比当前元素大的元素，因此我们需要维护一个**单调递减栈**
2. 初始化栈为空，`answer` 数组为全 `-1`
3. 遍历 `nums` 数组，对于每个元素 `nums[i]`：
   - 若栈为空，直接入栈（存储元素索引）
   - 若栈非空，比较栈顶索引对应元素与 `nums[i]` 的大小：
     - 若栈顶索引对应元素小于 `nums[i]`，则弹出栈顶索引，即当前栈顶索引对应元素的右侧第一个比它大的元素已找到。更新 `answer[栈顶索引]` 为 `nums[i]`，继续比较新的栈顶索引
     - 若栈顶索引对应元素大于等于 `nums[i]`，则 `nums[i]` 入栈（注意：这里允许相等，因为我们需要找到右侧第一个**比它大**的元素）
...
class Solution {
    public int[] nextGreaterElement(int[] nums) {
        ...
        Deque<Integer> stack = new ArrayDeque<>(); // 存储元素索引，保持栈内元素索引对应的 nums 元素单调递减
        for (int i = 0; i < n; i++) {
...
1. 我们需要找出一个区间中，所能达到的最大高度，可记为 `h`，这个区间的左右边界可记为 `l`, `r`
2. 创建一个栈，用于存储元素索引，保持栈内元素索引对应的 `heights` 元素单调递增
3. 由于单调递增，栈顶元素的前一个一定是它的左边第一个比它小的元素，即为 `l`
4. 当栈顶元素出栈时，即找到了右边第一个它小的元素，当前元素的索引即为 `r`

以 `[2, 1, 5, 6, 2, 3]` 为例，0(2)表示索引为0的元素2：
