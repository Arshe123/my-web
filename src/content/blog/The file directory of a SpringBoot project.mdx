...
import FileDirectory from '@/components/posts/FileDirectory.astro';
...
1. **JwtTokenInterceptor**：jwt令牌校验的拦截器，用于拦截处理请求信息，校验前端请求中的jwt令牌。
```java
@Component
@Slf4j
...
2. **GlobalExceptionHandler**：全局异常处理类，用于处理项目中抛出的异常。
```java
@Slf4j
@ControllerAdvice
